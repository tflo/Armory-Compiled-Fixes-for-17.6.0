--[[
    ...
    Revision: 228 2022-11-10T18:06:00Z
...
function ArmoryLookupFrame_OnLoad(self)
    ...
    self:SetTitle(ARMORY_LOOKUP_TITLE);

    self.Inset:SetPoint("TOPLEFT", self, "BOTTOMLEFT", 4, 344);
    self.Inset:SetPoint("BOTTOMRIGHT", self, "BOTTOMRIGHT", -4, 33);
...
function ArmoryLookupTradeSkillDropDown_Initialize()
    ...
    table.wipe(skills);
    for skill in pairs(LOOKUP_SKILLS) do
...
function ArmoryLookupFrame_Update()
    ...
    -- ScrollFrame update
    FauxScrollFrame_Update(ArmoryLookupScrollFrame, numLines, LOOKUP_LINES_DISPLAYED, ARMORY_LOOKUP_HEIGHT);
...
function ArmoryLookupFrame_SendRequest()
    ...
    if ( id == ARMORY_LOOKUP_TYPE.LOOKUP_RECIPE ) then
        ...
        version = "2";
        message = strjoin(ARMORY_LOOKUP_SEPARATOR, LOOKUP_SKILLS[skill], exact, search);
...
function ArmoryLookupFrame_GetSkillName(skill)
    if ( skill == LOOKUP_SKILLS.ARMORY_TRADE_ALCHEMY ) then
        return ARMORY_TRADE_ALCHEMY;
    elseif ( skill == LOOKUP_SKILLS.ARMORY_TRADE_BLACKSMITHING ) then
        return ARMORY_TRADE_BLACKSMITHING;
    elseif ( skill == LOOKUP_SKILLS.ARMORY_TRADE_COOKING ) then
        return ARMORY_TRADE_COOKING;
    elseif ( skill == LOOKUP_SKILLS.ARMORY_TRADE_ENCHANTING ) then
        return ARMORY_TRADE_ENCHANTING;
    elseif ( skill == LOOKUP_SKILLS.ARMORY_TRADE_ENGINEERING ) then
        return ARMORY_TRADE_ENGINEERING;
    elseif ( skill == LOOKUP_SKILLS.ARMORY_TRADE_JEWELCRAFTING ) then
        return ARMORY_TRADE_JEWELCRAFTING;
    elseif ( skill == LOOKUP_SKILLS.ARMORY_TRADE_LEATHERWORKING ) then
        return ARMORY_TRADE_LEATHERWORKING;
    elseif ( skill == LOOKUP_SKILLS.ARMORY_TRADE_POISONS ) then
        return ARMORY_TRADE_POISONS;
    elseif ( skill == LOOKUP_SKILLS.ARMORY_TRADE_TAILORING ) then
        return ARMORY_TRADE_TAILORING;
    elseif ( skill == LOOKUP_SKILLS.ARMORY_TRADE_INSCRIPTION ) then
    ...
    if ( search == Armory:GetGuildInfo("player") and dbEntry and dbEntry:Contains("Professions") ) then
        for profession in pairs(dbEntry:GetValue("Professions")) do
            ...
            if ( profession == ARMORY_TRADE_ALCHEMY ) then
                skill = LOOKUP_SKILLS.ARMORY_TRADE_ALCHEMY;
            elseif ( profession == ARMORY_TRADE_BLACKSMITHING ) then
                skill = LOOKUP_SKILLS.ARMORY_TRADE_BLACKSMITHING;
            elseif ( profession == ARMORY_TRADE_COOKING ) then
                skill = LOOKUP_SKILLS.ARMORY_TRADE_COOKING;
            elseif ( profession == ARMORY_TRADE_ENCHANTING ) then
                skill = LOOKUP_SKILLS.ARMORY_TRADE_ENCHANTING;
            elseif ( profession == ARMORY_TRADE_ENGINEERING ) then
                skill = LOOKUP_SKILLS.ARMORY_TRADE_ENGINEERING;
            elseif ( profession == ARMORY_TRADE_FIRST_AID ) then
                skill = LOOKUP_SKILLS.ARMORY_TRADE_FIRST_AID;
            elseif ( profession == ARMORY_TRADE_JEWELCRAFTING ) then
                skill = LOOKUP_SKILLS.ARMORY_TRADE_JEWELCRAFTING;
            elseif ( profession == ARMORY_TRADE_LEATHERWORKING ) then
                skill = LOOKUP_SKILLS.ARMORY_TRADE_LEATHERWORKING;
            elseif ( profession == ARMORY_TRADE_POISONS ) then
                skill = LOOKUP_SKILLS.ARMORY_TRADE_POISONS;
            elseif ( profession == ARMORY_TRADE_TAILORING ) then
                skill = LOOKUP_SKILLS.ARMORY_TRADE_TAILORING;
            elseif ( profession == ARMORY_TRADE_INSCRIPTION ) then
                skill = LOOKUP_SKILLS.ARMORY_TRADE_INSCRIPTION;
            end
