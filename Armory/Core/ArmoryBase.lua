--[[
    ...
    Revision: 151 2023-05-14T11:36:43Z
...
function Armory:InitializeMenu()
    if ( ARMORY_DROPDOWNMENU_MENU_LEVEL == 2 ) then
        ...
        ArmoryDropDownMenu_AddButton({text=ARMORY_SHARE_LABEL, hasArrow=1, notCheckable=true, owner=Armory.menu});
    end
end
...
function Armory:IsDbCompatible()
    ...
    elseif ( dbVersion ~= self.dbVersion) then
        ...
        if ( dbVersion > self.dbVersion ) then
            ...
            -- convert from 50 to 51
        elseif ( dbVersion == 50 ) then
            for realm in pairs(ArmoryDB) do
                for character in pairs(ArmoryDB[realm]) do
                    ...
                    if ( entry.Professions ) then
                        for _, data in pairs(entry.Professions) do
                            data.InvSlots = nil;
                        end
                    end
                end
            end
            for _, v in pairs(ArmoryShared) do
                if ( v.Professions ) then
                    for _, data in pairs(v.Professions) do
                        data.SubClasses = nil;
                        data.InvSlots = nil;
                    end
                end
        ...
        for _, profile in ipairs(self:Profiles()) do
            ...
            if ( #items > 0 ) then
                ...
                for _, item in ipairs(items) do
                    if ( not mode and self:GetConfigMailVerboseCount() ) then
                ...
                if ( not mode and not self:GetConfigMailVerboseCount() ) then
                    self:PrintWarning(format(ARMORY_CHECK_MAIL_SHORT_MESSAGE, profile.character, profile.realm, #items, floor(maxDays)));
                end
            end

            if ( mode ~= 2 and maxDays < 30 ) then
                local hasContainer = self:ContainerExists(ARMORY_MAIL_CONTAINER);
                ...
                if ( self:GetConfigMailCheckVisit() and not (self:GetConfigMailExcludeVisit() or (mode == 1 and self:GetConfigMailHideLogonVisit())) ) then
                    if ( hasContainer ) then
                        ...
                        if ( days >= 30 - maxDays and days < 31 ) then
                            self:PrintWarning(format(ARMORY_MAIL_VISIT_WARNING, profile.character, profile.realm, floor(days)));
                        end
                    else
                        self:PrintWarning(format(ARMORY_MAIL_VISIT_WARNING2, profile.character, profile.realm));
                    end
                end

                if ( hasContainer and self:GetConfigMailCheckCount() and not (mode == 1 and self:GetConfigMailHideLogonCount()) ) then
        ...
        for i = 1, numSlots do
            ...
            if ( daysLeft and floor(daysLeft) <= maxDays ) then
                ...
                if ( name ) then
                    ...
                    table.insert(mailItems, {name=name, link=link, count=count, left=floor(daysLeft), ignored=ignored});
                end
            end
        end
    end
        ...
        for _, profile in ipairs(self:Profiles()) do
            ...
            if ( (self:GetConfigGlobalItemCount() or self:IsConnectedRealm(profile.realm)) and (self:GetConfigCrossFactionItemCount() or _G.UnitFactionGroup("player") == self:UnitFactionGroup("player")) ) then
                ...
                for k, v in ipairs(result) do
                    if ( v.count > 0 ) then
                        table.insert(multipleItemCounts[k], {name=name, tier=v.tier, count=v.count, bags=v.bags, bank=v.bank, reagentBank=v.reagentBank, mail=v.mail, auction=v.auction, void=v.void, perSlot=v.perSlot, mine=mine});
                    elseif ( suspended ) then
                        table.insert(multipleItemCounts[k], {name=name, tier=v.tier, count=0});
                    end
                end
            end
        end
...
function Armory:GetQualityTierFromName(name)
    if (name) then
        return name:match("|A:.+Tier(%d)");
    end
end
