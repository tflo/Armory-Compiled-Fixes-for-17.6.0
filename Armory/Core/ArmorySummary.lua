...
local function DisplayQuests(tooltip, characterInfo)
    ...
    local numEntries;
    local questTitleText, level, isHeader, isCollapsed, isComplete, frequency, questID, label, questTag;
    local color;
        ...
        for _, character in ipairs(self:CharacterList(realm)) do
            ...
            if ( self:GetConfigSummaryBags() and self:HasInventory() ) then
                ...
                for i = 1, numLines do
                    ...
                    if ( isHeader and id >= BACKPACK_CONTAINER and id <= NUM_TOTAL_EQUIPPED_BAG_SLOTS ) then
...
function Armory:InitializeSummary(tooltip, anchorFrame)
    if ( self.summaryEnabled and not ((self.summary and self.summary:IsShown()) or ArmoryDropDownList1:IsVisible()) ) then
        ...
        SharedTooltip_SetBackdropStyle(GameTooltip, TOOLTIP_BACKDROP_STYLE_DEFAULT);
    ...
    for _, realmInfo in ipairs(summary) do
        ...
        for _, characterInfo in ipairs(realmInfo.Characters) do
            ...
            if ( not collapsed[realm] ) then
                local checked = (realm == currentProfile.realm and character == currentProfile.character);
...
function Armory:HideSummary(force)
    ...
    if ( not self.summary.locked ) then
