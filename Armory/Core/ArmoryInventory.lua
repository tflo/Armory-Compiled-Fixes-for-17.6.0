--[[
    ...
    Revision: 204 2023-02-21T19:10:16Z
...
local function SetSlotInfo(id, index, texture, count, quality, link, expires, equipable, slotId, ignorable)
    ...
    local tinker, anchor;
    if ( link and ((id >= BANK_CONTAINER and id <= NUM_BAG_SLOTS) or (id > NUM_TOTAL_EQUIPPED_BAG_SLOTS and id <= NUM_TOTAL_EQUIPPED_BAG_SLOTS + NUM_BANKBAGSLOTS)) and IsEquippableItem(link) ) then
...
function Armory:SetContainer(id)
    ...
    local link, texture;
    if ( id > BACKPACK_CONTAINER and id <= NUM_TOTAL_EQUIPPED_BAG_SLOTS + NUM_BANKBAGSLOTS ) then
    ...
    if ( not self:IsLocked(itemContainer) ) then
        ...
        elseif ( id == ARMORY_REAGENTBANK_CONTAINER ) then
            if ( _G.IsReagentBankUnlocked() ) then
                for i = 1, C_Container.GetContainerNumSlots(REAGENTBANK_CONTAINER) do
                    ...
                    if ( info and info.iconFileID ) then
                        SetSlotInfo(id, nextSlot, info.iconFileID, info.stackCount, info.quality, info.hyperlink, nil, nil, i);
        ...
        elseif ( id == ARMORY_REAGENTBANK_CONTAINER ) then
            numSlots = _G.IsReagentBankUnlocked() and C_Container.GetContainerNumSlots(REAGENTBANK_CONTAINER) or 0;
...
function Armory:GetInventoryContainerInfo(id, unit)
    ...
    if ( id == ARMORY_EQUIPMENT_CONTAINER ) then
        ...
        numSlots = INVSLOT_LAST_EQUIPPED;
    elseif ( dbEntry ) then
...
function Armory:GetInventoryContainerInfoEx(id, unit)
    ...
    elseif ( containerName ) then
        ...
        if ( id > NUM_TOTAL_EQUIPPED_BAG_SLOTS ) then
            prefix = ARMORY_BANK_CONTAINER_NAME.." #"..(id - NUM_TOTAL_EQUIPPED_BAG_SLOTS).." - ";
        elseif ( id > 0 ) then
            prefix = "#"..id.." - ";
        end
...
function Armory:ScanInventoryItems(items, reagentScan)
    ...
    local id, itemCount, name, link, itemString, qualityTier;
    local itemContainer;
    local buildCache;
    ...
    local saveResult = function(result, id, count)
        ...
        elseif ( self:GetConfigShowItemCountPerSlot() ) then
            ...
            local perSlot, where;
            if ( id >= BACKPACK_CONTAINER and id <= NUM_TOTAL_EQUIPPED_BAG_SLOTS ) then
                perSlot = result.perSlot.bags;
                where = tostring(id);
            elseif ( id == BANK_CONTAINER ) then
                perSlot = result.perSlot.bank;
                where = "0";
            elseif ( id > NUM_TOTAL_EQUIPPED_BAG_SLOTS and id <= NUM_TOTAL_EQUIPPED_BAG_SLOTS + NUM_BANKBAGSLOTS ) then
                perSlot = result.perSlot.bank;
                where = tostring(id - NUM_TOTAL_EQUIPPED_BAG_SLOTS);
            end
            if ( perSlot ) then
        ...
        elseif ( id >= BACKPACK_CONTAINER and id <= NUM_TOTAL_EQUIPPED_BAG_SLOTS ) then
            result.bags = result.bags + count;
        elseif ( id == BANK_CONTAINER or (id > NUM_TOTAL_EQUIPPED_BAG_SLOTS and id <= NUM_TOTAL_EQUIPPED_BAG_SLOTS + NUM_BANKBAGSLOTS) ) then
    ...
    for k, item in ipairs(items) do
        ...
        qualityTier = self:GetQualityTierFromName(name);
        result[k] = { name=name or UNKNOWN, item=itemString, tier=qualityTier, count=0, bags=0, bank=0, reagentBank=0, mail=0, auction=0, void=0 };

        for i = 1, #ArmoryInventoryContainers do
            id = ArmoryInventoryContainers[i];
            if ( (reagentScan and ((id >= BACKPACK_CONTAINER and id <= NUM_TOTAL_EQUIPPED_BAG_SLOTS) or id == ARMORY_REAGENTBANK_CONTAINER)) or (not reagentScan and not self:IsDummyContainer(id)) ) then
                itemContainer = InventoryContainerName(id);
...
function Armory:FindInventoryItem(itemList, ...)
    ...
    for i = 1, #ArmoryInventoryContainers do
        ...
        elseif ( id > NUM_TOTAL_EQUIPPED_BAG_SLOTS and id <= NUM_TOTAL_EQUIPPED_BAG_SLOTS + NUM_BANKBAGSLOTS ) then
            link = self:GetContainerItemLink(BANK_CONTAINER, NUM_BANKGENERIC_SLOTS + id - NUM_TOTAL_EQUIPPED_BAG_SLOTS);
        else
            link = nil;
        end
        if ( link ) then
