--[[
    ...
    Revision: 197 2022-10-30T09:57:47Z
...
function ArmoryBuffButton_UpdateAnchors(buttonName, index)
    ...
    if ( (index > 1) and (mod(index, BUFFS_PER_ROW) == 1) ) then
        -- New row
        buff:SetPoint("BOTTOM", _G[buttonName..(index - BUFFS_PER_ROW)], "TOP", 0, -BUFF_ROW_SPACING);
    elseif ( index == 1 ) then
        buff:SetPoint("TOPRIGHT", ArmoryBuffFrame, "TOPRIGHT", 0, 0);
    else
        buff:SetPoint("RIGHT", _G[buttonName..(index - 1)], "LEFT", -BUFF_COL_SPACING, 0);
    end
end
...
function ArmoryDebuffButton_UpdateAnchors(buttonName, index)
    ...
    if ( (index > 1) and (mod(index, BUFFS_PER_ROW) == 1) ) then
        ...
        buff:SetPoint("BOTTOM", _G[buttonName..(index - BUFFS_PER_ROW)], "TOP", 0, -BUFF_ROW_SPACING);
    elseif ( index == 1 ) then
        buff:SetPoint("TOPRIGHT", ArmoryBuffFrame, "TOPRIGHT", 0, rows * (BUFF_ROW_SPACING + buffHeight));
    else
        buff:SetPoint("RIGHT", _G[buttonName..(index - 1)], "LEFT", -BUFF_COL_SPACING, 0);
   end
end
