--[[
    ...
    Revision: 197 2022-11-19T21:41:37Z
...
local SOCIALFRAME_SUBFRAMES = { "ArmoryFriendsListFrame", "ArmoryIgnoreListFrame", "ArmoryEventsListFrame" };
...
function ArmorySocialFrame_ShowSubFrame(frameName)
    for index, value in pairs(SOCIALFRAME_SUBFRAMES) do
        _G[value]:Hide();
        if ( value == SOCIALFRAME_SUBFRAMES[ArmoryPanelTemplates_GetSelectedTab(ArmorySocialFrame)] ) then
            _G[value]:Show();
        end
    end
end
...
function ArmorySocialFrameTab_OnClick(self)
    ArmoryPanelTemplates_SetTab(ArmorySocialFrame, self:GetID());
    ArmorySocialFrame_ShowSubFrame();
    PlaySound(SOUNDKIT.IG_CHARACTER_INFO_TAB);
end

function ArmorySocialFrame_OnLoad(self)
    self:SetPortraitToAsset("Interface\\FriendsFrame\\FriendsFrameScrollIcon");
    ...
    ArmoryPanelTemplates_SetNumTabs(self, 3);
    ArmoryPanelTemplates_SetTab(self, 1);
end

function ArmorySocialFrame_OnShow(self)
    PlaySound(SOUNDKIT.IG_CHARACTER_INFO_TAB);
    ArmoryPanelTemplates_SetTab(self, ArmoryPanelTemplates_GetSelectedTab(self));
...
function ArmoryFriendsListFrame_OnShow(self)
    ArmorySocialFrame:SetTitle(FRIENDS_LIST);
    ArmoryFriendsList_Update();
end

function ArmoryFriendsList_Update()
    local numFriends = Armory:GetNumFriends();
    local showScrollBar = (numFriends > FRIENDS_TO_DISPLAY);
    ...
    -- ScrollFrame stuff
    FauxScrollFrame_Update(ArmoryFriendsListScrollFrame, numFriends, FRIENDS_TO_DISPLAY, ARMORY_SOCIALFRAME_FRIEND_HEIGHT);
end

function ArmoryIgnoreListFrame_OnShow(self)
    ArmorySocialFrame:SetTitle(IGNORE_LIST);
    ArmoryIgnoreList_Update();
end
...
function ArmoryIgnoreList_Update()
    ...
    -- ScrollFrame stuff
    FauxScrollFrame_Update(ArmoryIgnoreListScrollFrame, numIgnores, IGNORES_TO_DISPLAY, ARMORY_SOCIALFRAME_IGNORE_HEIGHT);
end
...
local INVITESTATUS_INFO = {
    ...
    [Enum.CalendarStatus.Confirmed] = { name=CALENDAR_STATUS_CONFIRMED, color=GREEN_FONT_COLOR },
    [Enum.CalendarStatus.Available] = { name=CALENDAR_STATUS_ACCEPTED, color=GREEN_FONT_COLOR },
    [Enum.CalendarStatus.Declined] = { name=CALENDAR_STATUS_DECLINED, color=RED_FONT_COLOR },
    [Enum.CalendarStatus.Out] = { name=CALENDAR_STATUS_OUT, color=RED_FONT_COLOR },
    [Enum.CalendarStatus.Standby] = { name=CALENDAR_STATUS_STANDBY, color=ORANGE_FONT_COLOR },
    [Enum.CalendarStatus.Invited] = { name=CALENDAR_STATUS_INVITED, color=NORMAL_FONT_COLOR },
    [Enum.CalendarStatus.Signedup] = { name=CALENDAR_STATUS_SIGNEDUP, color=GREEN_FONT_COLOR },
    [Enum.CalendarStatus.NotSignedup] = { name=CALENDAR_STATUS_NOT_SIGNEDUP, color=NORMAL_FONT_COLOR },
    [Enum.CalendarStatus.Tentative] = { name=CALENDAR_STATUS_TENTATIVE, color=ORANGE_FONT_COLOR },
};
...
function ArmoryEventsListFrame_OnShow(self)
    ArmorySocialFrame:SetTitle(EVENTS_LABEL);
    ArmoryEventsList_Update();
end
...
function ArmoryEventsList_GetEventDetail(eventIndex)
    ...
    elseif ( calendarType == "RAID_LOCKOUT" ) then
        ...
        elseif ( calendarType == "GUILD_EVENT" and inviteType == Enum.CalendarStatus.Signedup ) then
            local inviteStatusInfo = INVITESTATUS_INFO[inviteStatus] or INVITESTATUS_INFO["UNKNOWN"];
            if ( inviteStatus == Enum.CalendarStatus.NotSignedup or inviteStatus == Enum.CalendarStatus.Signedup ) then
...
function ArmoryEventsList_Update()
    ...
    local showScrollBar = (numEvents > EVENTS_TO_DISPLAY);
    local eventTime, isOldEvent, title, status;
    local dateTimeText, titleText, titleHiddenText, statusText;
    local eventButton;
    local fullDate;
    ...
    -- ScrollFrame stuff
    FauxScrollFrame_Update(ArmoryEventsListScrollFrame, numEvents, EVENTS_TO_DISPLAY, ARMORY_SOCIALFRAME_EVENT_HEIGHT);
end
