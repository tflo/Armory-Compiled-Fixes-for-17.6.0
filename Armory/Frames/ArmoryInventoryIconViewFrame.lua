--[[
    ...
    Revision: 244 2023-05-14T11:20:33Z
...
local MAX_CONTAINER_COLUMNS = 7;
local MAX_CONTAINER_ITEMS = 160;
local CONTAINER_OFFSET_X = 22;
local CONTAINER_OFFSET_Y = -5;
local CONTAINER_ROW_HEIGHT = 37;
local NUM_CONTAINER_COLUMNS = 4;

local VOID_CONTAINER_COLUMNS = 10;
local VOID_CONTAINER_ROW_HEIGHT = 26;
...
function ArmoryInventoryIconViewFrame_ShowContainer(containerFrame)
    ...
    local rowHeight = CONTAINER_ROW_HEIGHT;

    if ( Armory:GetInventoryBagLayout() and id >= BACKPACK_CONTAINER and id <= NUM_TOTAL_EQUIPPED_BAG_SLOTS + NUM_BANKBAGSLOTS ) then
    ...
    elseif ( Armory:GetInventoryBagLayout() and id == ARMORY_VOID_CONTAINER ) then
        ...
        columns = VOID_CONTAINER_COLUMNS;
        rowHeight = VOID_CONTAINER_ROW_HEIGHT;
        buttonBaseName = containerName.."VoidItem";
        buttonTemplate = "ArmoryInventoryFrameVoidItemButtonTemplate";
    else
        columns = MAX_CONTAINER_COLUMNS;
    end

    if ( numSlots > MAX_CONTAINER_ITEMS ) then
        numSlots = MAX_CONTAINER_ITEMS;
    ...
    if ( isPlusTwoBag and not isCollapsed ) then
        for i = 1, 2 do
            ...
            if ( i == 1 ) then
                itemButton:SetPoint("TOPLEFT", containerName.."Label", "BOTTOMLEFT", CONTAINER_OFFSET_X, 0);
            else
    ...
    for i = numSlots + 1, MAX_CONTAINER_ITEMS do
        index = offset + i;
        ...
        for i = 1, numSlots do
            ...
            if ( texture ) then
                if ( Armory:GetInventoryBagLayout() ) then
                    ...
                    if ( id == ARMORY_VOID_CONTAINER ) then
                        -- transpose
                        local voidRows = ARMORY_VOID_STORAGE_MAX / VOID_CONTAINER_COLUMNS;
                        local rowStart = mod(index - 1, voidRows) * VOID_CONTAINER_COLUMNS;
                        local colIndex = mod(floor((index - 1) / voidRows), VOID_CONTAINER_COLUMNS);
    ...
    containerFrame:SetHeight((rows * rowHeight) + label:GetHeight() - CONTAINER_OFFSET_Y);
    containerFrame:Show();
...
function ArmoryInventoryIconViewFrame_CheckContainerSize(label, size)
    local numSlots;
    if ( size > MAX_CONTAINER_ITEMS ) then
        numSlots = MAX_CONTAINER_ITEMS;
