--[[
    ...
    Revision: 206 2022-11-08T12:36:18Z
...
function AGB:UpdateGuildInfo()
    local tabardData = C_GuildInfo.GetGuildTabardInfo("player");
    if ( tabardData ) then
        self:SetValue("TabardInfo",
            tabardData.backgroundColor:GenerateHexColor(),
            tabardData.borderColor:GenerateHexColor(),
            tabardData.emblemColor:GenerateHexColor(),
            tabardData.emblemFileID,
            tabardData.emblemStyle
        );
    end
...
function AGB:GetGuildTabardInfo(dbEntry)
    local backgroundHexColor, borderHexColor, emblemHexColor, emblemFileID, emblemIndex = self:GetValue(dbEntry, "TabardInfo");
    if ( backgroundHexColor ) then
        return {
            backgroundColor = CreateColorFromHexString(backgroundHexColor),
            borderColor = CreateColorFromHexString(borderHexColor),
            emblemColor = CreateColorFromHexString(emblemHexColor),
            emblemFileID = emblemFileID,
            emblemStyle = emblemIndex
        };
    end
end
...
function AGB:Find(...)
    ...
    local list = {};
    local dbEntry, container, items, name, link, itemCount, realmName, text;
